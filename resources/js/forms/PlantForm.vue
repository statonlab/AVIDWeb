...
import SiteDefaults from '../helpers/Storage/SiteDefaults'
...
export default {
  ...
  watch: {
    ...
    date(date) {
      const measurement_date = date ? moment(date).format('YYYY-MM-DD') : null
      SiteDefaults.set(this.plot.site_id, {measurement_date})
      this.form.date = measurement_date
    },

    'form.quadrant': {
      handler(quadrant) {
        SiteDefaults.set(this.plot.site_id, {quadrant})
      },
    },

    'form.species_id': {
      handler(species_id) {
        SiteDefaults.set(this.plot.site_id, {species_id})
        this.form.errors.clear('species_id')
      },
    },

    'form.plant_type_id': {
      handler(plant_type_id) {
        SiteDefaults.set(this.plot.site_id, {plant_type_id})
        this.loadSpecies()
      },
    },
  },
  ...
  async mounted() {
    const defaults = SiteDefaults.get(this.plot.site_id)

    this.date = defaults.measurement_date ? moment(defaults.measurement_date).toDate() : null
    this.form.setDefault({
      ...defaults,
      ...{date: defaults.measurement_date || null},
    })
  ...
  methods: {
    async loadTypes() {
      ...
      try {
        ...
        this.plants             = data
        this.form.plant_type_id = this.form.plant_type_id === '' ? data[0].id : this.form.plant_type_id
